...
| Tool | Para qué |
|------|----------|
| `mcp__jira__jira_get_issue` | Obtener issue por clave (estado, tipo, assignee, priority, fechas) |
| `mcp__jira__jira_search_issues` | Buscar por JQL |
| `mcp__jira__jira_search_issues_in_project` | Issues recientes en un proyecto |
| `mcp__jira__jira_create_issue` | Crear issue (Task, Bug, Story, Epic, Subtarea) |
| `mcp__jira__jira_update_issue` | Actualizar campo: status, duedate, startdate, labels, estimate, assignee, summary, priority, **epic** (asigna a épica) |
| `mcp__jira__jira_add_worklog` | Registrar horas trabajadas |
| `mcp__jira__jira_add_comment` | Agregar comentario |
...
| `mcp__jira__jira_get_project_issue_types` | Tipos disponibles en proyecto |
| `mcp__jira__jira_quality_metrics` | Métricas de calidad (completionRate, onTimeDelivery, avgCycleTime, reworkRate) |
| `mcp__jira__jira_team_performance` | Performance del equipo basado en worklogs |
| `mcp__jira__jira_incentives` | Calcular incentivos |
| `mcp__jira__jira_weekly_plan` | Plan semanal (activos, vencidos, WIP, higiene) |
| `mcp__jira__jira_weekly_update` | Actualizar reporte semanal |
| `mcp__jira__confluence_search` | Buscar páginas en Confluence |

### Tools Google Sheets / BBDD

| Tool | Para qué |
|------|----------|
| `mcp__jira__sheets_read` | Leer datos de Google Sheets |
| `mcp__jira__sheets_write` | Escribir datos |
| `mcp__jira__sheets_analyze` | Analizar cuellos de botella, vencidos, carga |
| `mcp__jira__sheets_metadata` | Metadatos de hojas |
| `mcp__jira__sheets_find_columns` | Buscar columnas por nombre |
| `mcp__jira__sheets_suggest_improvements` | Sugerir mejoras basadas en datos |
| `mcp__jira__jira_bbdd_create` | Crear issue + fila en BBDD |
| `mcp__jira__jira_bbdd_update` | Actualizar issue + BBDD |
| `mcp__jira__jira_bbdd_append_comment` | Comentar + registrar en BBDD |
| `mcp__jira__jira_bbdd_create_sheet_from_template` | Crear hoja desde plantilla |
| `mcp__jira__jira_bbdd_sync_epic` | Sincronizar épica con BBDD |
| `mcp__jira__jira_bbdd_audit_jira_admin` | Auditar Jira Admin |
| `mcp__jira__jira_bbdd_match_jira_admin` | Match jira_admin por responsable |
...
**mcp__jira__jira_update_issue** — soporta estos field aliases:
...
Usar `mcp__jira__jira_get_issue("<issue_key>")` para obtener: estado, tipo, subtareas (si es padre), tiempo registrado, asignado, vencimiento, épic asociado.
...
Si es padre (Task/Story con subtareas) → `mcp__jira__jira_search_issues` con JQL `parent = <issue_key>` para obtener hijos.
...
Para issues en la BBDD (Google Sheets), usar `mcp__jira__jira_bbdd_create` que también crea fila en la hoja de seguimiento.
...
```
Luego vincular a la épica con `mcp__jira__jira_update_issue(issueKey="<nuevo_issue>", field="epic", value="<epic_key>")`. **NO usar `jira_link_to_epic`** — esa tool crea un link genérico, no asigna la épica correctamente.
...
```
mcp__jira__jira_bbdd_append_comment(
  issueKey="<issue_key>",
...
1. Consultar Jira (`mcp__jira__jira_get_issue`) para obtener estado, assignee, fechas actuales
2. Consultar subtareas existentes (`mcp__jira__jira_search_issues` con JQL `parent = <issue_key>`)
